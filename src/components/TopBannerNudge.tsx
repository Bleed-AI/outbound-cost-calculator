 ...
 * Hidden while total is below the threshold; reappears if user crosses it again
 * after raising volume (unless dismissed during this session).
 */
export function TopBannerNudge({ total }: TopBannerNudgeProps) {
  const threshold = PRICING.packageNudgeThreshold
  const [dismissed, setDismissed] = useState(false)

  // Restore session-level dismissal so the banner doesn't reappear on re-render.
  useEffect(() => {
    try {
  ...
  const visible = total >= threshold && !dismissed
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="sticky top-0 z-40 w-full backdrop-blur-md bg-[rgba(177,19,15,0.12)] border-b border-[rgba(177,19,15,0.35)]"
        >
